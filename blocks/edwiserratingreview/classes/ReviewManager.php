...
class ReviewManager {
     ...
     * Short course card design for rating and review (rnr)
     */
    public function get_short_design_enrolmentpage($forid) {
        global $CFG , $DB;
        $context = \context_course::instance($forid);
        $data = $DB->get_record('block_instances', array('blockname' => 'edwiserratingreview', 'parentcontextid' => $context->id));
        if (!$data) {
        ...
        $html = '';
        $html .= '<div class="d-flex align-items-center rating-short-design">';
        $html .= "<div class='inner d-flex align-items-center'>";
        $html .= "<span class='small-info-regular avgratingstat'><b>$data->averagerating</b> ({$data->totalcount} Reviews)&nbsp;</span>";
        $html .= $data->avergeratingstar . "</div>";
        $html .= "</div>";
